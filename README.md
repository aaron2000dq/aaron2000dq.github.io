...
- 富力中心附近彩排：<https://aaron2000dq.github.io/?mode=nearby>

在 iPad Safari 中打开并横屏使用：

1. 首次打开时保持联网，等待首页显示“离线地图已缓存”。
2. 允许定位与相机权限，然后可通过 Safari 分享菜单“添加到主屏幕”。
3. 默认入口为杭州正式路线；加 `?mode=nearby` 才进入余杭富力中心四门彩排。
...
地点、坐标、解锁半径、文案、参考照片和评分阈值集中在 `src/config/story.ts`。正式版与彩排版运行时坐标统一使用 WGS-84；每张手绘地图通过一组真实路线锚点和画面锚点完成局部配准。
